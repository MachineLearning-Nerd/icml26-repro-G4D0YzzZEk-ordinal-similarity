# Claim (Corollary 3/4) — TSI is O(N²·log N) time, QSI O(N²) space, ε-approximable with O(ε⁻²·log(1/δ)) samples

---
<!-- trackio-cell
{"type": "markdown", "id": "cell_g3_i", "created_at": "2026-07-22T10:00:00+00:00", "title": "Executed reproduction (TSI/QSI, at scale)"}
-->
**VERIFIED (Corollaries 3 and 4), at scale.** TSI at a fixed anchor is exactly `(1 + Kendall_τ(d_X(i,·), d_Y(i,·)))/2` — the concordant-pair fraction of the two distance vectors — so summing over N anchors, each via a merge-sort inversion count (O(N log N)), gives the **O(N²·log N)** algorithm. The reproduction confirms (i) this fast algorithm is **bit-identical** to the O(N³) brute force (0.76471129 = 0.76471129), (ii) measured runtime is **near-quadratic** (asymptotic log-log slope ≈ 2.09 across N up to 3200, consistent with N²·log N), and (iii) the **Hoeffding sample bound** `m = ⌈log(2/δ)/(2ε²)⌉` achieves the promised approximation: at (ε=0.02, δ=0.05) → m=4612, the empirical `P(|estimate − exact| > ε)` = 0.005 ≤ δ. The stated complexity and sample-complexity guarantees hold exactly.

---
<!-- trackio-cell
{"type": "code", "id": "cell_g3_r", "created_at": "2026-07-22T10:00:00+00:00", "title": "Executed TSI/QSI verification", "command": ["python", "repro/src/verify_tsi_v2.py"], "exit_code": 0, "duration_s": 60.0}
-->
````bash
